...
def rule_d_idle_deposits(holdings: dict, *, value: dict, **_) -> list[Candidate]:
    ...
    if deposits >= _IDLE_DEPOSIT_MIN and 'savings' not in held:
        return [Candidate(
            ...
            reason=f'Holds ~KES {deposits / 1_000_000:.1f}M in deposits with no dedicated '
                   f'savings product. Move the idle balance into a yield-bearing account.',
...
def rule_e_lending_only(holdings: dict, *, value: dict, **_) -> list[Candidate]:
    ...
    if lending and 'mobile' not in held:
        return [Candidate(
            ...
            reason='Active borrower not yet on mobile banking. Digital servicing is the obvious next step.',
